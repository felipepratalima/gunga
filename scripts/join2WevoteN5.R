...
metaphlan2Location <- wevoteLocation %>% paste0("/wevote_metaphlan2.tsv")
tippLocation <- wevoteLocation %>% paste0("/wevote_tipp.tsv")
...
if (file.exists(metaphlan2Location)) {
  currentDf <- read_tsv(metaphlan2Location, col_names = c("seq", "metaphlan2"))
...
if (file.exists(tippLocation)) {
  currentDf <- read_tsv(tippLocation, col_names = c("seq", "tipp"))
...
## Define output file
outputFile <- wevoteLocation %>% paste0("/wevote5_joined.csv")
