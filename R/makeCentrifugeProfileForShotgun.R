...
makeCentrifugeProfileForShotgun <- function(rawCentrifugeProfileDf = NULL, rawCentrifugeClassificationsDf = NULL, taxdumprObject = NULL) {
  ...
  gungaProfile$dnaCentrifugePercentual <- gungaProfile$numUniqueReads / numberOfSequences
  gungaProfile <- gungaProfile %>% select(speciesId = standardId,
                                                    dnaCentrifugePercentual,
                                                    dnaCentrifugeScoreMedian = scoreMedian,
                                                    dnaCentrifugeScoreCI = scoreCI,
                                                    dnaCentrifugeHitLengthMedian = hitLengthMedian,
                                                    dnaCentrifugeHitLengthCI = hitLengthCI,
                                                    dnaCentrifugeQueryLengthMedian = queryLengthMedian,
                                                    dnaCentrifugeQueryLengthCI = queryLengthCI,
                                                    dnaCentrifugeNumberOfMatchesMedian = numberOfMatchesMedian,
                                                    dnaCentrifugeNumberOfMatchesCI = numberOfMatchesCI)
...
# rawCentrifugeProfileDf <- loadRawCentrifugeProfile("inst/testdata/sh/centrifuge_report.tsv")
# rawCentrifugeClassificationsDf <- loadRawCentrifugeClassifications("inst/testdata/sh/centrifuge_classifications.tsv")
